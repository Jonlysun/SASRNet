...
def state_dict_reload(state_dict):
    ...
    for key in state_dict.keys():
        if key.startswith('SR_Warping'):
            new_key = key.replace('SR_Warping', 'feature_extraction')
        elif key.startswith('Depth_esti'):
            new_key = 'depth_estimation_mapping' + '.' + key
        elif key.startswith('IP_Warping.pcd_align_front'):
            new_key = key.replace('IP_Warping', 'feature_refinement')
        elif key.startswith('IP_Warping.RB11_ip_merge') or key.startswith('IP_Warping.conv11_ip'):
            new_key = key.replace('IP_Warping', 'depth_estimation_mapping')
        elif key.startswith('IP_Warping.RB22_ip_merge') or key.startswith('IP_Warping.conv22_ip'):
            new_key = key.replace('IP_Warping', 'mutal_referenced_SR')
        elif key.startswith('IP_Warping.RB33_ip_merge') or key.startswith('IP_Warping.conv33_ip'):
            new_key = key.replace('IP_Warping', 'mutal_referenced_SR')
        elif key.startswith('IP_Warping.merge_background_conv_lv'):
            new_key = key.replace('IP_Warping', 'feature_refinement')
        else:
            new_key = key.replace('IP_Warping', 'mutal_referenced_SR')
...
def get_parser(additional_commands=None):
    ...
    parser.add_argument("--cmd", type=str, default="retrain", choices=commands)
...
class WorkerObjects(object):
    ...
    def get_optimizer(self, net):
        optimizer = self.optim_f(net)
        self.lr_scheduler_f = lr_scheduler.CosineAnnealingLR_Restart(
                optimizer, config.T_period, eta_min=config.eta_min,
                    restarts=config.restarts, weights=config.restart_weights)
    ...
    def get_lr_scheduler(self, optimizer):
        ...
        return (
            None
            if self.lr_scheduler_f is None
            else self.lr_scheduler_f
        )

    def set_optimizer_lr_scheduler(self, optimizer, lr_scheduler):
        self.optim_f.load_state_dict(optimizer)
        self.lr_scheduler_f.load_state_dict(lr_scheduler)
        return self.optim_f,  self.lr_scheduler_f
...
class Worker(object):
    ...
    def do_cmd(self, args, worker_objects):
        ...
        if args.cmd == "retrain":
            ...
            self.eval_iters(
                worker_objects, iters=args.iter, net_root=args.eval_net_root, cmd='eval'
            )
    ...
    def eval_iters(self, worker_objects, iters=None, net_root=None, cmd='eval'):
        ...
        for iter_str in iters:
            if iter_str in net_paths:
                ...
                logging.info(f"[EVAL] loading net for iter {iter_str}: {net_path}")
                state_dict = torch.load(str(net_path))

                net.load_state_dict(state_dict)

                self.eval(iter, net, eval_sets, cmd=cmd)
            else:
                logging.info(f"[EVAL] no network params for iter {iter_str}")

    def eval(self, iter, net, eval_sets, epoch="x", cmd='eval'):
        for eval_set_idx, eval_set in enumerate(eval_sets):
            logging.info("")
            logging.info("=" * 80)
            logging.info(f"Evaluating set {eval_set.name}")
            self.eval_set(iter, net, eval_set_idx, eval_set, epoch=epoch, cmd=cmd)

    def eval_set(self, iter, net, eval_set_idx, eval_set, epoch="x", cmd='eval'):
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(eval_loader):
                if cmd == 'retrain' and batch_idx == 4:
                    break
    ...
    def train(self, worker_objects, resume=False):
        ...
        if resume and state_path.exists():
            ...
            # iter = 33441 + 1
            # state_dict = torch.load("./experiments/last-200000-ssimalpha-final/net_0000000000033441.params")
            # net.load_state_dict(state_dict)
            net.load_state_dict(state["state_dict"])
            optimizer.load_state_dict(state["optimizer"])
            lr_scheduler.load_state_dict(state["lr_schedulers"])
            # print(state["lr_schedulers"])
            # lr_scheduler.load_state_dict(state["lr_schedulers"])
            # optimizer, lr_scheduler = worker_objects.set_optimizer_lr_scheduler(state["optimizer"], state["lr_schedulers"])
            # optimizer = worker_objects.get_optimizer(net)
            # lr_scheduler = worker_objects.get_lr_scheduler(optimizer)
            if config.use_gan == True:
                self.train_loss.load_discriminator(state_path)
        ...
        self.stopwatch.start("total")
        self.stopwatch.start("data")
        use_pretrained_sr = config.use_pretrained_sr
        pretrained_requires_grad = config.pretrained_requires_grad
        for iter, data in zip(iter_range, train_loader):
            ...
            if use_pretrained_sr == True and pretrained_requires_grad == False and iter == 5:
                net.pretrained_change()
            ...
            self.stopwatch.start("update_learning_rate")
            lr_scheduler.step()
            self.stopwatch.stop("update_learning_rate")
            ...
            if iter < 128 or iter % train_set.logging_rate == 0:
                ...
                message = '{:.3e}'.format(current_lr[0])
                # message = str(current_lr[0])
                self.train_iter_messages.append(message)
                # lr_scheduler.step()
                # current_lr = lr_scheduler.get_lr()
                # message = str(current_lr[0])
                # self.train_iter_messages.append(message)
            ...
            if do_save or iter == iter_range[-1] or (iter+1) % config.manul_save_model ==0:
                # store network
                ...
                state_dict = {
                    ...
                    "lr_schedulers":worker_objects.get_lr_scheduler(optimizer).state_dict()
                }
                if config.use_gan == True:
                    state_dict["discriminator"] = self.train_loss.get_discriminator().state_dict()
                if torch.cuda.is_available():
            ...
            # eval network
            if do_eval or (iter+1) % config.manul_save ==0:
                self.eval(
                    iter, net, eval_sets, epoch=self.save_frequency.n_resets, cmd='retrain'
                )
                net = net.to()
                net = net.cuda()
                net.train()
