...
bool is_valid_projection_patch(int h, int w, float proj_d, const depthmap_t& dm,
                         int h_from, int h_to, int w_from, int w_to, float bwd_depth_thresh,
                         bool invalid_depth_to_inf) {
    bool in_domain = proj_d > 0 && w >= w_from && h >= h_from && w < w_to && h < h_to;
    if (!in_domain) {
        return false;
    }
    ...
    for (int tgt_h = 0; tgt_h < patch_height; ++tgt_h) {
        for (int tgt_w = 0; tgt_w < patch_width; ++tgt_w) {
            ...
            for (int vidx = 0; vidx < n_views; ++vidx) {
                ...
                if (!is_valid_projection_patch(
                        src_h, src_w, proj_d, src_dms[vidx], h_from, h_to,
                        w_from, w_to, bwd_depth_thresh, invalid_depth_to_inf)) {
                    continue;
                }
                int idx = (vidx * patch_height + tgt_h) * patch_width + tgt_w;
                valid_map_masks[idx] = 1;
                maps[idx * 2 + 0] = (2 * (src_wf - w_from) / (patch_width - 1)) - 1;
                maps[idx * 2 + 1] = (2 * (src_hf - h_from) / (patch_height - 1)) - 1;
...
//                maps[idx * 2 + 1] = (2 * src_hf / (src_height - 1)) - 1;
    ...
    return std::make_tuple(create_arrayN<float>(maps, {n_views, patch_height, patch_width, 2}),
